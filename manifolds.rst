...
While a line is the simplest topological manifold, a circle is the next simplest.
A circle cannot be uniquely mapped back to a line through a single chart.
While it is possible to find a function which will map each point on the circle to the real line, by gluing the ends together, it will not be invertible.

Instead the entire circle can be covered my an atlas consisting of four charts.

.. rubric:: Sphere

The sphere can be totally mapped by an atlas of two charts, but as with a circle it cannot be covered by a single chart, as one point always needs to be omitted from the chart.
...
Suppose :math:`a` and :math:`b` are two numbers and :math:`x^i = x^i(\mu)` is a curve through :math:`P`, then at :math:`P`,
